...
test('VideoPosePlayback supports a hero variant that trims chrome and constrains the stage height for the analysis first screen', () => {
  ...
  assert.match(source, /watch\(\s*\(\) => props\.variant,\s*async \(\) => \{\s*await drawFrame\(\)\s*\}\s*\)/)
  ...
  assert.match(source, /if \(index !== \(props\.selectedFrameIndex \?\? 0\)\) \{\s*emit\('update:selectedFrameIndex', index\)/)
  assert.match(source, /emit\('update:selectedFrameIndex', index\)/)
