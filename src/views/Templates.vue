...
import { ArrowLeft, Check, Loader2, PencilLine, Trash2, X } from 'lucide-vue-next'
...
const currentVideoTemplateProfile = computed(() => currentVideoAnalysis.value?.templateProfile ?? null)
const currentImage = computed(() => analysisStore.currentAnnotatedImage || analysisStore.currentImage)
...
const saveTemplateEdits = async () => {
  ...
  const currentTemplate = templates.value.find(template => template.id === editingTemplateId.value)
  if (!currentTemplate) {
  ...
  <div class="templates-page">
    ...
    <div class="templates-page__content">
      <header class="templates-page__header">
        <Button variant="ghost" size="icon" class="templates-page__back" @click="goBack">
          <ArrowLeft class="h-5 w-5" />
        </Button>

        <div class="templates-page__heading">
          <p class="templates-page__eyebrow">模板管理</p>
      ...
      <section class="templates-page__grid">
          ...
          <CardContent class="templates-panel__content">
            <div v-if="currentAnalysisSnapshot" class="templates-analysis">
              ...
              <div class="templates-analysis__meta">
                ...
                <div v-if="currentVideoTemplateProfile" class="templates-analysis__metric templates-analysis__metric--video">
                  <span class="templates-analysis__label">视频采样</span>
                  <strong>{{ currentVideoTemplateProfile.samplesUsed }} 帧</strong>
                </div>
                <div v-if="currentVideoTemplateProfile" class="templates-analysis__metric templates-analysis__metric--video">
                  <span class="templates-analysis__label">动作覆盖</span>
                  <strong>{{ formatTemplateCoverage(currentVideoTemplateProfile.coverage) }}</strong>
                </div>
              </div>

              <div v-if="!currentVideoTemplateProfile" class="templates-video-required">
              ...
              <form v-else class="templates-form" @submit.prevent="void saveCurrentAnalysisAsTemplate()">
                <div class="templates-form__row">
                  <div class="templates-form__field">
                    <Label for="template-name">球星名称</Label>
                    <Input id="template-name" v-model="form.name" placeholder="例如：斯蒂芬·库里" />
                  </div>

                  <div class="templates-form__field">
                    <Label for="template-team">球队 / 标签</Label>
                    <Input id="template-team" v-model="form.team" placeholder="例如：金州勇士" />
                  </div>
                </div>
                ...
                <div v-if="saveError" class="templates-form__feedback templates-form__feedback--error">
                  {{ saveError }}
                </div>
                <div v-else-if="saveSuccess" class="templates-form__feedback templates-form__feedback--success">
                  {{ saveSuccess }}
                </div>
        ...
        <Card class="templates-panel templates-panel--library">
          ...
          <CardContent class="templates-panel__content">
            <div v-if="loadError" class="templates-library__feedback templates-library__feedback--error">
              {{ loadError }}
            </div>
              ...
              >
                <div class="templates-library__topline">
                  ...
                  <div class="templates-library__header-actions">
                    <span class="templates-library__badge">#{{ template.id }}</span>
                    <Button
                      variant="outline"
                      size="sm"
                      class="templates-library__edit-trigger"
                      :disabled="deletingTemplateId === template.id || pendingDeleteTemplateId === template.id"
                    ...
                    <Button
                      ...
                      :disabled="deletingTemplateId === template.id || isUpdatingTemplate"
                ...
                >
                  <div class="templates-delete-confirm__copy">
                    <p class="templates-delete-confirm__title">确认删除这个模板？</p>
                    <p class="templates-delete-confirm__body">
                      「{{ template.name }}」删除后不会再参与球星对比。这个操作不能撤销。
                    </p>
                  </div>
                ...
                <template v-if="editingTemplateId === template.id">
                  <form class="templates-edit-form" @submit.prevent="void saveTemplateEdits()">
                    <div class="templates-form__row">
                      <div class="templates-form__field">
                        <Label :for="`template-edit-name-${template.id}`">球星名称</Label>
                        <Input
                      ...
                      <div class="templates-form__field">
                        <Label :for="`template-edit-team-${template.id}`">球队 / 标签</Label>
                        <Input
                    ...
                    <div class="templates-form__field">
                      <Label :for="`template-edit-description-${template.id}`">模板说明</Label>
                      <textarea
                    ...
                    <div class="templates-edit-form__actions">
                      <Button type="submit" :disabled="!canSaveTemplateEdits || isUpdatingTemplate">
                        <Loader2 v-if="isUpdatingTemplate" class="h-4 w-4 animate-spin" />
                ...
                <template v-else>
                  ...
                  <div class="templates-library__meta">
                    ...
                    >
                      整段视频模板 · {{ template.templateProfile.samplesUsed }} 帧 · 覆盖 {{ formatTemplateCoverage(template.templateProfile.coverage) }}
                    </span>
                    <span v-else class="templates-library__mode templates-library__mode--legacy">
                      旧单帧模板 · 建议删除后用完整视频重建
                    </span>
                    <span>{{ template.angles.length }} 个角度特征</span>
                    <span>{{ template.poseData.keypoints.length }} 个姿态关键点</span>
                  </div>
                </template>
              </article>
            </div>
...
.templates-analysis__metric--video {
  ...
  background:
    linear-gradient(135deg, color-mix(in srgb, var(--accent-color) 11%, transparent), transparent 72%),
...
.templates-video-required {
  ...
  background:
    linear-gradient(135deg, color-mix(in srgb, var(--color-warning, #d28b18) 12%, transparent), transparent 70%),
...
.templates-delete-confirm {
  ...
  background:
    linear-gradient(135deg, color-mix(in srgb, var(--color-danger-bg) 96%, transparent), transparent 68%),
    color-mix(in srgb, var(--card-bg) 94%, var(--background));
...
.templates-delete-confirm__submit {
  ...
  background:
    linear-gradient(135deg, var(--color-danger), var(--color-danger-hover)) !important;
  color: #fff !important;
  font-weight: 800;
...
.templates-delete-confirm__submit:hover {
  ...
  background:
    linear-gradient(135deg, var(--color-danger-hover), color-mix(in srgb, var(--color-danger-hover) 84%, black)) !important;
  color: #fff !important;
}
