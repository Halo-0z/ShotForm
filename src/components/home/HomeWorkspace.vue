...
const visualizedImage = computed(
...
const currentVideoFrame = computed(() => {
    ...
    return (
        analysis.frames[analysisStore.currentVideoFrameIndex] ??
...
const isAnalysisWorkspace = computed(
    () => activeModule.value === "analysis" && (analysisStore.isLoading || hasAnalysis.value),
)
...
<template>
    ...
    >
        <div class="home-container" :class="{ 'workspace-container': isAnalysisWorkspace }">
            <section
                class="analysis-hero-shell"
                :class="{ compact: isAnalysisWorkspace, focused: showUpload }"
            >
                ...
                <div class="analysis-hero-copy">
                    <header class="home-header" :class="{ compact: isAnalysisWorkspace }">
                        ...
                        <label class="auto-ai-toggle">
                            <input
                                ...
                                type="checkbox"
                                @change="
                                    analysisStore.setAutoAiAnalysisEnabled(
                    ...
                    <nav
                        class="module-nav"
                        :class="{ compact: isAnalysisWorkspace }"
                        aria-label="home modules"
                    >
                        <HomeModuleGem
                            ...
                            :disabled="
                                (mod.key === 'compare' || mod.key === 'suggestion') && !hasAnalysis
                            "
            ...
            <main class="module-content">
                <section
                    v-show="activeModule === 'upload'"
                    class="upload-section"
                    :class="{ 'active-upload': showUpload }"
                >
                        ...
                        <div v-if="!showUpload" class="upload-choice-grid">
                            ...
                            <Card class="choice-card video">
                                <CardContent class="choice-card-content">
                                    ...
                                    <p class="choice-text">
                                        支持裁剪片段并做 MediaPipe
                                        动态骨骼分析，更适合看节奏、关键帧和整段动作。
                                    </p>
                                    <Button
                                        size="lg"
                                        class="choice-btn"
                                        @click="openUpload('video')"
                                    >
                                        选择视频
                                        <ArrowRight class="ml-2 h-4 w-4" />
                                    </Button>
                            ...
                            <Card class="image-card" :class="{ secondary: currentVideoAnalysis }">
                                <CardHeader class="analysis-card-header">
                                    <div class="analysis-title-row">
                                        ...
                                        <Badge
                                            v-if="currentVideoAnalysis && currentVideoFrame"
                                            variant="secondary"
                                            class="phase-badge"
                                        >
                                ...
                                <CardContent>
                                    <div
                                        ...
                                        :class="{
                                            tall: !currentVideoAnalysis,
                                            focus: currentVideoAnalysis,
                                        }"
                                    >
                                        <img
                                ...
                                <CardHeader class="analysis-card-header">
                                    <div class="analysis-title-row">
                                        ...
                                        <Badge variant="secondary" class="phase-badge">
                                            {{ formatTime(currentVideoAnalysis.trimStartMs) }} -
                                            {{ formatTime(currentVideoAnalysis.trimEndMs) }}
                                        </Badge>
                                    </div>
                                </CardHeader>
                                <CardContent>
                                    <div class="video-overview-top">
                                        <div class="video-summary-chip">
                                            ...
                                            <strong
                                                class="video-summary-value"
                                                data-allow-copy="true"
                                            >
                                                {{
                                                    getShotTypeName(
                                                        currentVideoAnalysis.overallShotType,
                                                    )
                                                }}
                                            </strong>
                                        </div>
                                        <div class="video-summary-chip">
                                            ...
                                            >
                                                {{
                                                    (
                                                        currentVideoAnalysis.overallShotTypeConfidence *
                                                        100
                                                    ).toFixed(1)
                                                }}%
                                            </strong>
                                    ...
                                    <div class="keyframe-strip filmstrip">
                                        <button
                                            ...
                                            :class="{
                                                active:
                                                    index === analysisStore.currentVideoFrameIndex,
                                            }"
                                            @click="selectVideoFrame(index)"
                                        >
                                            <img
                                                :src="frame.annotatedImageData || frame.imageData"
                                                alt="关键帧缩略图"
                                            />
                            ...
                            <Card class="data-card featured">
                                <CardHeader class="analysis-card-header">
                                    <div class="analysis-title-row">
                                        <div>
                                            <CardTitle>{{
                                                currentVideoAnalysis ? "当前帧诊断" : "分型评审"
                                            }}</CardTitle>
                                            <p class="card-caption">
                                                {{
                                                    currentVideoAnalysis
                                                        ? "结合当前选中关键帧，给出更像教练讲解的判断。"
                                                        : "根据当前单帧姿态，给出分型判断和训练提示。"
                                                }}
                                            </p>
                                        </div>
                                ...
                                <CardContent>
                                    ...
                                    <div
                                        v-if="getDisplayedSummary(analysisStore.currentAnalysis)"
                                    ...
                                    <div class="reasons-list">
                                        <p
                                            v-for="(reason, index) in getCoachReasons(
                                                analysisStore.currentAnalysis,
                                            )"
                                            :key="reason"
                                            class="reason-item"
