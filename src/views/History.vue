...
<template>
    <div class="history-page">
                    ...
                    <header v-if="historyList.length > 0" class="history-archive__toolbar">
                        ...
                        <div class="history-archive__filters">
                            <div class="history-archive__search">
                                ...
                                <input
                                    v-model="searchQuery"
                                    class="history-archive__search-input"
                                    type="text"
                                    placeholder="搜索投篮分型..."
                                />
                            </div>
                        ...
                        >
                            ...
                            >
                                <div class="history-group__toggle">
                                    <ChevronDown
                                        v-if="isExpanded(group.sourceIdentifier)"
                                        class="w-4 h-4"
                                    />
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div class="history-session-row__main">
                                        <div class="history-session-row__preview">
                                            <img
                                                :src="record.annotatedImagePath"
                                                class="history-session-row__preview-image"
                                                alt="历史记录"
                                            />
                                        </div>
                                                ...
                                                <Badge
                                                    :variant="
                                                        getShotTypeBadgeVariant(
                                                            record.analysis.shotType,
                                                        )
                                                    "
                                                >
                                                    {{ getShotTypeName(record.analysis.shotType) }}
                                                </Badge>
                                                <span class="history-session-row__result"
                                                    >分析结果已保存</span
                                                >
                                            </div>
                                            <p class="history-session-row__summary">
                                                可以继续查看详情或恢复分析流程。
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="history-session-row__actions">
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            class="history-session-row__resume"
                                ...
                                <ProgressChart :records="group.records" />
                            </div>
                        </div>
                    </div>

                    <div
                        v-if="hasMore && !searchQuery && !shotTypeFilter"
                        class="history-archive__more"
                    >
                        <Button
                            variant="outline"
        ...
        <ConfirmDialog
            ...
            title="确定要删除这条记录吗？"
            description="删除后无法恢复，但你可以重新上传分析。"
            confirm-label="删除"
            variant="danger"
            :loading="isDeleting"
...
.dark .history-archive__search-input {
...
.dark .history-archive__select {
    background: rgba(255, 255, 255, 0.08);
    border-color: rgba(168, 180, 196, 0.28);
    color-scheme: dark;
}

.history-archive__select option {
