  ...
  <section class="upload-workbench">
    ...
    <section class="upload-workbench__main upload-workbench__deck">
      <div class="upload-workbench__mode-strip" aria-label="上传工具选择">
        ...
        <Button :variant="uploadMode === 'image' ? 'segmented-active' : 'secondary'" size="sm"
          @click="uploadMode = 'image'">
          <ImageIcon class="h-4 w-4" />
          图片
        </Button>
        <Button :variant="uploadMode === 'video' ? 'segmented-active' : 'secondary'" size="sm"
      ...
      <div v-if="isBrowserPreviewMode" class="upload-workbench__browser-note" data-browser-preview-note role="note">
      ...
      <div class="upload-workbench__surface">
        <ImageUpload v-if="uploadMode === 'image'" :key="workbenchKey"
          :desktop-analysis-available="!isBrowserPreviewMode" :loading="isBusy" @image-loaded="handleImageLoaded" />
        <VideoUpload v-else :key="workbenchKey" compact :desktop-analysis-available="!isBrowserPreviewMode"
          :loading="isBusy" @video-loaded="handleVideoLoaded" />
      </div>
